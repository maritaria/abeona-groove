...
import groove.explore.strategy.*;
...
public enum StrategyValue implements ParsableValue {
    ...
    BFS(
            ...
            "This strategy first generates all possible transitions from each " + "open state, and then continues in a breadth-first fashion."),
    /**
    ...
    DFS(
            ...
            "This strategy first generates all possible transitions from each " + "open state, and then continues in a depth-first fashion."),
    /**
     * Linear strategy.
     */
    LINEAR(
            "linear",
            "Linear Exploration",
            "This strategy chooses one transition from each open state. " + "The transition of choice will be the same within one " + "incarnation of Groove."),
    /**
    ...
    RANDOM(
            ...
            "This strategy chooses one transition from each open state. " + "The transition is chosen randomly."),
    /**
    ...
    RETE(
            ...
            "This strategy finds all possible transitions from the Rete " + "network, and continues in a depth-first fashion using " + "virtual events when possible. Rete updates are applied " + "accumulatively"),
    /**
    ...
    CONDITIONAL(
            ...
            "This strategy performs a conditional breadth-first exploration. " + "If a given rule is applicable in a newly reached state, it " + " is not explored further. " + "All other states are explored normally."),
    /**
     * Node bound conditional strategy.
     */
    CONDITIONAL_NODE_BOUND(
            "cnbound",
            "Conditional Exploration (Node Bound)",
            "This strategy performs a conditional breadth-first exploration. " + "If the number of nodes in a newly reached state exceeds a " + "given bound, it is not explored further. " + "All other states are explored normally."),
    /**
     * Edge bound conditional strategy.
     */
    CONDITIONAL_EDGE_BOUND(
            "cebound",
            "Conditional Exploration (Edge Bound)",
            "This strategy performs a conditional breadth-first exploration. " + "If the number of edges in a newly reached state exceeds a " + "given bound, it is not explored further. " + "All other states are explored normally."),
    /**
    ...
    LTL_BOUNDED(
            ...
            "Nested Depth-First Search for a given LTL formula," + "using incremental bounds based on graph size or rule applications"),
    /**
     * Bounded LTL model checking strategy.
     */
    LTL_POCKET(
            ...
            "Nested Depth-First Search for a given LTL formula," + "using incremental bounds based on graph size or rule applications" + "and optimised to avoid reexploring connected components ('pockets')"),
    /**
     * Minimax strategy.
     */
    ...
    public Template<Strategy> getTemplate() {
        switch (this) {
            ...
            case CONDITIONAL:
                return new MyTemplate2<Rule, Boolean>(new PSequence(new POptional(
                        "!",
                        "mode",
                        EncodedRuleMode.NEGATIVE,
                        EncodedRuleMode.POSITIVE),
            ...
            case CONDITIONAL_NODE_BOUND:
                return new MyTemplate1<Integer>(new PNumber("node-bound"), "node-bound", new EncodedInt(0, -1)) {
            ...
            case CONDITIONAL_EDGE_BOUND:
                return new MyTemplate1<Map<TypeLabel, Integer>>(new PSeparated(new PSequence(new PIdentifier(
                        "edge-bound"),
                        new PLiteral(">", "edge-bound"),
                        new PNumber("edge-bound")), new PLiteral(",", "edge-bound")),
                        "edge-bound",
                        new EncodedEdgeMap()) {
            ...
            case LTL_BOUNDED:
                SerializedParser boundParser = new PSeparated(new PChoice(
                        new PIdentifier("rule"),
                        new PNumber("value")), new PLiteral(",", "comma"));
                SerializedParser parser = new PSequence(boundParser, new PLiteral(";", "semi"), new PAll("prop"));
                return new MyTemplate2<String, Boundary>(parser,
                        "prop",
            ...
            case MINIMAX:
                return new MyTemplate5<Integer, Integer, List<Rule>, Rule, Integer>(new PSequence(new PNumber(
                        ...
                        new PLiteral(","),
                        new PSeparated(
                                new PIdentifier("enabled-rule-names"), /*delimiter*/
                                new PLiteral(";", "enabled-rule-names")),
                        new PLiteral(","),
                        new PIdentifier("start-max"),
                        new PLiteral(","),
                        new PIdentifier("minmax-rule"),
                        new PLiteral(","),
                        new PNumber("minmax-rule-parameter-index")),
                        "heuristic-parameter-index",
                        new EncodedInt(0, Integer.MAX_VALUE),
                    ...
                    public Strategy create(Object[] arguments) {
                        ...
                        return new MinimaxStrategy(parindex, searchdepth, labels, minmaxrule, minmaxparam);
                    }
                };
     ...
     */
    public final static EnumSet<StrategyValue> LTL_STRATEGIES = EnumSet.of(LTL, LTL_BOUNDED, LTL_POCKET);
    /**
     ...
     */
    public final static EnumSet<StrategyValue> DEVELOPMENT_ONLY_STRATEGIES = EnumSet.of(
            RETE,
            RETE_LINEAR,
            RETE_RANDOM,
            MINIMAX);
